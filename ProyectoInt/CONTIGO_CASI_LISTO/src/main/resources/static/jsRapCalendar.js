	...
	this.ShowMonth = function(year,month,del){
		...
		for(var n = 0;n < fd;n++)
		 ...
		 for(var n = 1;n <= dim;n++){
            ...
            if(currentDay>=n && currentMonth === this.curMonth){
	
	        if(currentDay===n) {
            
			   d.append('<div class="item"><li data-toggle="modal" data-target="#staticBackdrop" id="estrella_' + (this.curMonth+1) + '' +  n + '" value="estrella' + (this.curMonth+1) + '_' +  n + '" class="fa fa-star star-color" style="font-size:60px;color:' +  map.get(n) + '"><div style="z-index:1;margin-top:-35px;font-size:15px;padding-top:-10px;color:white;">'+ n +'</div></li></div>')
			
			} else  {
				
				d.append('<div disabled="true" class="item" style="pointer-events:none;cursor: default;"><li data-toggle="modal" id="estrella_' + (this.curMonth+1) + '' +  n + '" value="estrella' + (this.curMonth+1) + '_' +  n + '" class="fa fa-star star-color" style="font-size:60px;color:' +  map.get(n) + ';pointer-events:none;cursor: default;"><div style="z-index:1;margin-top:-35px;font-size:15px;padding-top:-10px;color:white;pointer-events:none;cursor: default;">'+ n +'</div></li></div>')
							
			}
			
			} else {
				
			d.append('<div disabled="true" class="item" style="pointer-events:none;cursor: default;"><li data-toggle="modal" id="estrella_' + (this.curMonth+1) + '' +  n + '" value="estrella' + (this.curMonth+1) + '_' +  n + '" class="fa fa-star star-color" style="font-size:60px;color:black;pointer-events:none;cursor: default;"><div style="z-index:1;margin-top:-35px;font-size:15px;padding-top:-10px;color:white;pointer-events:none;cursor: default;">'+ n +'</div></li></div>')
