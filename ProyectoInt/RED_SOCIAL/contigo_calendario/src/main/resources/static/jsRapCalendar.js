	...
	this.ShowMonth = function(year,month,del){
		...
		for(var n = 0;n < fd;n++)
		 ...
		 for(var n = 1;n <= dim;n++){
			...
			let d = $('<td>')
			.append('<div class="item"><li type="button" data-toggle="modal" data-target="#staticBackdrop" id="estrella_' + (this.curMonth+1) + '_' +  n + '" value="estrella_' + (this.curMonth+1) + '_' +  n + '" class="fa fa-star star-color" style="font-size:60px;color:' +  map.get(n) + '"></li></div>')
			.append('<div style="z-index:1;margin-top:-40px;font-size:15px;padding-top:-10px;color:black;">'+ n +'</div>')
			.appendTo(tr);
